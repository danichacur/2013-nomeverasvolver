...
int tratamiento_recurso(t_pers_por_nivel * personaje, char* str_nivel, t_niveles_sistema *nivel, int32_t *quantum);
void tratamiento_asesinato(int32_t nivel_fd, t_pers_por_nivel* personaje, char* mensaje, char* str_nivel);
                ...
                for (i = 0; i <= fdmax; i++) {
                        if (FD_ISSET(i, &read_fds)) {
                                ...
                                if (recibirMensaje(i, &tipoMensaje, &mensaje) != EXIT_SUCCESS) {
                                        ...
                                        log_info(logger_pla, "Nivel %s: Llego este mensaje: %s .", str_nivel,mensaje);
                                        log_info(logger_pla, "Nivel %s: se esperaba la respuesta de %d .",
                                                        str_nivel,fd_personaje_actual);
                                        log_info(logger_pla, "Nivel %s: se recibió el mensaje de %d .", str_nivel,i);
...
t_pers_por_nivel *planificar(t_niveles_sistema* nivel) {
        ...
        log_info(logger_pla, "Nivel %d: Sale %c de la lista de listos", nivel->nivel,
                        aux->personaje);
        ...
        case PER_movimiento_PLA: {
                ...
                if (t_mensaje == NIV_movimiento_PLA) {
                        ...
                        if (t_mensaje == NIV_enemigosAsesinaron_PLA){
                                ...
                                tratamiento_asesinato(nivel->fd,personaje,m_mensaje,str_nivel);
                        }else{
	...
	// log_info(logger_pla, "%s" Dani aca me devuelve una lista vacia y rompe, pero recibió el mensaje de que un enemigo asesino un personaje");
	  //  t_pers_por_nivel *aux;
			...
			t_list *p_listos = dictionary_get(listos, str_nivel);
                        ...
                        pthread_mutex_lock(&mutex_listos);
                        personaje = list_remove_by_condition(p_listos, (void*) _esta_personaje);
                        pthread_mutex_unlock(&mutex_listos);
                        ...
                        log_info(logger_pla,
                                        ...
                                        str_nivel, personaje->personaje);
                        enviarMensaje(personaje->fd, PLA_teMatamos_PER, "0");
                        destruir_personaje(personaje);
                        i++;
                }
...
void planificador_analizar_mensaje(int32_t socket_r,
		...
		log_info(logger_pla, "Nivel %s: Recibo el asesinato de: %s",
                                        str_nivel, mensaje);
                tratamiento_asesinato(mensaje,str_nivel);
                break;
        }*/
