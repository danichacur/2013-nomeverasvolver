...
t_list * desbloquear_personajes(t_list *recursos_obtenidos, char *str_nivel) {
        ...
        if (list_is_empty(p_bloqueados) || (p_bloqueados == NULL )) {
                log_info(logger,"DesbloquearPersonajes: No habia personajes que desbloquear");
        } else {
                int j = 0;
                                ...
                                if (i == socketEscucha) {
                                        ...
                                        if ((newfd = accept(socketEscucha,
                                                ...
                                                log_info(logger,"selectserver: new connection from %s on "
                                                                "socket %d", inet_ntoa(remoteaddr.sin_addr),
                                                                newfd);
                                        }
                                } else {
...
void orquestador_analizar_mensaje(int32_t sockett, enum tipo_paquete tipoMensaje,
                char* mensaje) {
        switch (tipoMensaje) {
...
//		recu = string_from_format("%c", mensaje[0]);
		...
		nuevo->retardo = atoi(n_mensaje[3]);
                ...
                log_info(logger, "Se crea el hilo planificador para el nivel (%d) recien conectado con fd= %d ",
                                nivel, sockett);
                pthread_t pla;
