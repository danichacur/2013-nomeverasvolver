...
void supr_pers_de_estructuras(int32_t sockett) {
	...
	if (aux == NULL ) {
		...
		if (aux2 == NULL ) {
			...
			//  log_info(logger,
			//        "EL personaje muerto se estaba planificando. Rompe todo!!!. O , termino el nivel correctamente");
			// }
			//}
		}
		if (aux2 != NULL ) {
...
void imprimir_lista(int32_t tipo, char *str_nivel) {
	...
	if (tipo == LISTA_LISTOS) {
		//lista de listos
		t_list *p_listos = dictionary_get(listos, str_nivel);
		log_info(logger_pla, "Nivel %s: Se imprime lista de personajes listos",
				str_nivel);
		pthread_mutex_lock(&mutex_listos);
		for (i = 0; i < list_size(p_listos); i++) {
			elemento = list_get(p_listos, i);
			cadena = transformarListaCadena(elemento->recursos_obtenidos);
			log_info(logger_pla,
...
//                nuevo->remain_distance = atoi(n_mensaje[4]);
	...
	case PER_conexionNivel_ORQ: {
		...
		// delegar la conexión al hilo del nivel correspondiente
		log_info(logger, "Nivel solicitado por el personaje %c: %d ", aux->simbolo,nivel);
		...
		if (p_monitor == NULL ) {
			...
			t_niveles_sistema* caso= list_find(niveles_del_sistema,
								(void*) _esta_nivel);

			item2->pos_recurso = caso->remain_distance;
			pthread_mutex_lock(&mutex_listos);
