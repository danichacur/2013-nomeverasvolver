...
void imprimir_lista(int32_t tipo, char *str_nivel){
	...
	//lista de listos
    t_list *p_listos = dictionary_get(listos, str_nivel);
	log_info(logger_pla, "Se imprime lista de personajes listos");
	pthread_mutex_lock(&mutex_listos);
	for (i =0;i<=list_size(p_listos);i++){
		elemento = list_get(p_listos, i);
		log_info(logger_pla, "Personaje: %c, posee los recursos:", elemento->personaje);
	}

	pthread_mutex_unlock(&mutex_listos);
	log_info(logger_pla, "Fin de impresión de la lista de personajes listos");
	}else{
	// lista de bloqueados
	t_list *p_bloqueados = dictionary_get(bloqueados, str_nivel);
	log_info(logger_pla, "Se imprime lista de personajes bloqueados");
	pthread_mutex_lock(&mutex_bloqueados);
		for (i =0;i<=list_size(p_bloqueados);i++){
		elemento = list_get(p_bloqueados, i);
		log_info(logger_pla, "Personaje: %c, bloqueado por: %c, posee los recursos:", elemento->personaje, elemento->recurso_bloqueo);
	}
	pthread_mutex_unlock(&mutex_bloqueados);
	log_info(logger_pla, "Fin de impresión de la lista de personajes bloqueados");
	}
